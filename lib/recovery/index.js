    ...
    function onGlobalErrorThresholdExceeded(appEvent) {
        ...
        try {
            ...
            events.notify('recovery.recoveryAttemptCompleted', {
                code,
                moduleName,
                // we pre-apply `moduleName` and `errorCount` to this function otherwise they will not be
                // in scope when then this function is executed by the subscriber of this event (i.e. lib/supervisor)
                fn: validateRecoveryAttempt.bind(null, { errorCount, moduleName })
            });
