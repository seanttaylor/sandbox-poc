...
export default function PluginHTTPMediaStrategyPost(sandbox) {
  ...
  function myPlugin(strategyConfiguration) {
    ...
    const defaultMediaType = IPostService(strategyConfiguration.textHTML);

    /* Map of valid HTTP request `Accept` header values */
    const strategyMap = {
      '*/*': defaultMediaType,
      'application/hal+json': IPostService(strategyConfiguration.applicationHAL),
    ...
    function setMediaType(mediaType) {
      if (!Object.keys(strategyMap).includes(mediaType)) {
        ...
        _currentMediaStrategy = IPostService(strategyMap['text/html']);
     ...
     * @param {Object} args - client-supplied arguments
     * @returns - the strategy return type
     */
    function setViewAuthnLevel(args) {
      return _currentMediaStrategy.setViewAuthnLevel(args);
    }
     ...
     * @returns - the strategy return type
     */
    async function getPostPreview(args) {
      return _currentMediaStrategy.getPostPreview(args);
    }
