...
export default function ChaosModule(sandbox) {
    ...
    function myPlugin({ chaosEnabled = 'false', scheduleTimeoutMillis = 0 }) {
        if (chaosEnabled === 'false') {
            console.warn('Environment variable (CHAOS_ENABLED) MUST be set true to run *OR* register chaos experiments.');
            return {
                getAllExperiments: () => ({}),
                runAllExperiments: () => ({}),
        ...
        /**************** EVENT REGISTRATION ****************/
        //TODO Change to 'application.chaos.experiment.registrationRequested' and expose on return `onExperimentRegistrationRequest` on the plugin API
