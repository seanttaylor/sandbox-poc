    ...
    function myPlugin(postService) {
         ...
         */
         async function exists(reqId) {
            const { exists: postExists, summary } = await postService.exists(reqId);
            const HALResponse = halson(summary);

            HALResponse
                .addLink('self', { href: postExists ? `/api/v1${reqId}`: null, rel: postExists ? 'post' : null })
                .addLink('curies', Curies())
                .addLink('sandbox:index', APIRootRelation())
                .addLink('sandbox:posts', [
                    PostRelation(),
                    (postExists ? DeletePostRelation(reqId) : DeletePostRelation()),
                ])
                .addLink('sandbox:users', UserRelation())
