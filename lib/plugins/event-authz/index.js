...
export default function pluginEventAuthz(box) {
    ...
    function validatePermissions(subscriberId, event) {
        ...
        // The subscriber has not been configured in the permissions file
        if (!Object.keys(permissions[event]).includes(subscriberId)) {
            console.error(`EventAuthzError.NotFound (${subscriberId}) => Could not validate event subscriber`);
            return false;
        }
        ...
        if (!permissions[event][subscriberId]) {
            console.error(`EventAuthzError.Unauthorized (${subscriberId}) => Subscriber not authorized for this event (${event}). See ./permissions.js`);
            return false;
        }
