...
export default function (box) {
  ...
  async function getPostById(id) {
    const [data] = await box.database.findOne({ id, collection: 'posts' });
  ...
  async function getAllPosts() {
    const data = await box.database.findAll('posts');
  ...
  */
  async function editPost({ id, body }) {
    const [existingRecord] = await box.database.findOne({
      id,
      collection: 'posts'
    });
    const [data] = await box.database.updateOne({
  ...
  async function deletePost(id) {
    return box.database.removeOne({ id, collection: 'posts' });
  }
  ...
  async function exists(id) {
    return box.database.exists({ id, collection: 'posts' });
  }
